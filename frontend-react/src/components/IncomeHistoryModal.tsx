...
const IncomeHistoryModal: React.FC<Props> = ({ transactions, currentCycle, formatAmount, onClose, onCycleDeleted }) => {
  ...
  const groups = useMemo<CycleGroup[]>(() => {
    ...
    // Derive the date-range label from min(created_at) and (firstCycleStart - 1d).
